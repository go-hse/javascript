...
cd /D %PARENT%
mkdir Sources
echo console.log^('Hello World!'^) > Sources\01_hello_world.js

:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:: GET JUPYTERS BY HTTPS
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

set REPO=javascript_introduction
...
mkdir %PARENT%\jupyter
set JUPYTER_PATH=%PARENT%\jupyter

deno upgrade rc
deno jupyter --install


:: write Deno kernel specs to AppData to add the --unstable feature (dlopen problem)
::  %APPDATA% includes Roaming
set "dt=%DATE:~6,4%.%DATE:~3,2%.%DATE:~0,2%%TIME:~0,2%:%TIME:~3,2%:%TIME:~6,2%"
(
echo {  "argv": ["%DENO_PATH%\\deno.exe", "jupyter", "--unstable", "--kernel", "--conn", "{connection_file}" ], "display_name": "Deno %dt%", "language": "typescript" }
) > %APPDATA%\jupyter\kernels\deno\kernel.json
...
echo :: set "JUPYTER_PATH=%PARENT%\jupyter"
echo start jupyter notebook %REPO%\notebooks
echo start code Sources
echo start "Javascript-Umgebung in %PARENT%" %comspec% /K
) > %PARENT%\start_javascript.bat
...
deno --version
deno Sources\01_hello_world.js
exit /b
